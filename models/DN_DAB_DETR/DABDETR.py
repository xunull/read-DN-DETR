...
class DABDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries,
        ...
        # 这两句是dab没有的
        # 论文中提高了这个
        # 第一个加一是因为论文中提到了一个unknown
        # 第二个-1 是因为最后一位是indicator
        # hidden_dim = 256  (92,255)
        ...
        if random_refpoints_xy:
            ...
            # sigmoid的反函数，得到特征图上的坐标形式
            self.refpoint_embed.weight.data[:, :2] = inverse_sigmoid(self.refpoint_embed.weight.data[:, :2])
            # xy不在更新，xy并不会跟随网络的变化而更新，但是在具体的 经历了各个decoder之后，还是会逐层更新的
            self.refpoint_embed.weight.data[:, :2].requires_grad = False

        # 图像特征到transformer内的投影
    ...
    def forward(self, samples: NestedTensor, dn_args=None):
        """
        dn_args的内容 targets, args.scalar, args.label_noise_scale, args.box_noise_scale, args.num_patterns

        args.scalar                 number of dn groups
        args.label_noise_scale      label noise ratio to flip
        args.box_noise_scale        box noise scale to shift and scale
        args.num_patterns           Anchor DETR中的pattern
        ...
        # default pipeline (300,4)
        embedweight = self.refpoint_embed.weight

        # dn 的前处理 prepare for dn
        # 生成noise gt 以及mask
        # embedweight [300,4] 是refpoint_embed的权重值
        # scr.size(0) 是batch size
        # self.label_enc 是Embedding(92,255)
        ...
        input_query_label, input_query_bbox, attn_mask, mask_dict = prepare_for_dn(dn_args, embedweight, src.size(0),
                                                                                   self.training, self.num_queries,
                                                                                   self.num_classes,
        ...
        # 第二个返回值是参考点
        hs, reference = self.transformer(self.input_proj(src), mask, input_query_bbox, pos[-1],
        ...
        # 多了一个mask_dict的返回值
        # mask_dict 用于计算去噪部分的loss，上面的两个output是正常的用于计算匹配部分的
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, mask_dict=None, return_indices=False):
        ...
        num_boxes = sum(len(t["labels"]) for t in targets)  # 所有的gt的数量
